...
```{r}
# covariates --------------------------------------------------------------
vekt_cov <- c('o_preop_vektskole', 'b_ant_vekt', 'p_alder_v_op', 'Female', 'b_beh_diab', 'smoke')
```


Let’s calculate the mean for each covariate by the treatment status.
There are large differences in base reg.age and diabetes.
...
mod_match_nearest <- matchit(formula =  o_preop_vektskole ~ a5_WL + p_alder_v_op +
                                Female + b_beh_diab + smoke,
                     method = "nearest", 
                     data = vekt_nomiss)

mod_match_optimal <- matchit(formula =  o_preop_vektskole ~ a5_WL + p_alder_v_op +
                                Female + b_beh_diab + smoke,
                     method = "optimal", 
                     data = vekt_nomiss)

mod_match_log   <- matchit(formula =  o_preop_vektskole ~ a5_WL + log(p_alder_v_op) +
                                Female + b_beh_diab + smoke,
                     method = "optimal", 
                     data = vekt_nomiss)
```
...
Nearest neigbour matching does not work very well.  However, the optimal matching method
results in balanced matching.  Also, taking log() of the age is another way to 
reach a balance in the PSM.


```{r}
summary(mod_match_nearest)
plot(mod_match_nearest)

summary(mod_match_optimal)
plot(mod_match_optimal)

summary(mod_match_log)
plot(mod_match_log)
```

There is no statistically distinguishable difference in the weight loss.

```{r}
dta_m <- match.data(mod_match_optimal)
dim(dta_m)  #[1] 1234  10                     1234 = 617*2

with(dta_m, t.test(a5_WL ~ o_preop_vektskole))
```

Using %TWL instead, gives similar results.
```{r}
